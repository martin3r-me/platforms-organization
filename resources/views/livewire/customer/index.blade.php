        ...
        <x-ui-page-sidebar title="Filter" width="w-80" :defaultOpen="true" side="left">
            <div class="p-6 space-y-6">
                <div>
                    ...
                    <x-ui-input-text name="search" wire:model.live="search" placeholder="Suche Kunden..." class="w-full" size="sm" />
                </div>
                <div>
                ...
                @foreach($this->customers as $customer)
                    <x-ui-table-row compact="true">
                        <x-ui-table-cell compact="true">
                            <a href="{{ route('organization.customers.show', $customer) }}" class="link">{{ $customer->code }}</a>
                        </x-ui-table-cell>
                        <x-ui-table-cell compact="true">{{ $customer->name }}</x-ui-table-cell>
                        <x-ui-table-cell compact="true">
                        ...
                        <x-ui-table-cell compact="true">
                            <x-ui-badge variant="{{ $customer->is_active ? 'success' : 'muted' }}">{{ $customer->is_active ? 'Aktiv' : 'Inaktiv' }}</x-ui-badge>
                        </x-ui-table-cell>
                    </x-ui-table-row>
                @endforeach
            </x-ui-table-body>
