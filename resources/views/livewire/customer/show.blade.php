<x-ui-page>
    ...
    <x-slot name="actionbar">
        <x-ui-page-actionbar :breadcrumbs="[
            ...
            ['label' => 'Kunden', 'href' => route('organization.customers.index')],
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="py-3 px-4 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            ...
                            <div class="text-sm font-medium text-[var(--ui-secondary)]">{{ $customer->created_at->format('d.m.Y H:i') }}</div>
                        </div>
                        <div class="py-3 px-4 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            <span class="text-xs text-[var(--ui-muted)]">Aktualisiert</span>
                            <div class="text-sm font-medium text-[var(--ui-secondary)]">{{ $customer->updated_at->format('d.m.Y H:i') }}</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <x-ui-page-container>
        ...
        <div class="space-y-6">
            ...
            {{-- Verknüpfte Objekte --}}
            @if($this->linkedContexts->count() > 0)
                <div class="bg-white rounded-lg border border-[var(--ui-border)] p-6">
                    <h2 class="text-lg font-semibold text-[var(--ui-secondary)] mb-4">Verknüpfte Objekte</h2>
                    <div class="space-y-4">
                        @foreach($this->linkedContexts as $group)
                            <div>
                                <h3 class="text-sm font-medium text-[var(--ui-muted)] uppercase tracking-wider mb-2">
                                    {{ $group['label'] }} ({{ $group['count'] }})
                                </h3>
                                <div class="space-y-1">
                                    @foreach($group['items'] as $item)
                                        <div class="flex items-center gap-2 py-1.5 px-3 bg-[var(--ui-muted-5)] rounded text-sm">
                                            <span class="font-medium text-[var(--ui-secondary)]">{{ $item['name'] }}</span>
                                            @if($item['percentage'])
                                                <span class="text-xs text-[var(--ui-muted)]">{{ number_format($item['percentage'], 0) }}%</span>
                                            @endif
                                            @if($item['is_primary'])
                                                @svg('heroicon-s-star', 'w-3.5 h-3.5 text-amber-500')
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
