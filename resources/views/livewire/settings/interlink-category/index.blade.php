<x-ui-page>
        ...
        <x-ui-page-sidebar title="Filter" width="w-80" :defaultOpen="true" side="left">
            <div class="p-6 space-y-6">
                <div>
                    ...
                    <x-ui-input-text name="search" wire:model.live="search" placeholder="Suche Kategorien..." class="w-full" size="sm" />
                </div>
                <div>
                ...
                @foreach($this->categories as $category)
                    <x-ui-table-row compact="true">
                        <x-ui-table-cell compact="true">
                            <code class="text-xs bg-[var(--ui-muted-5)] px-2 py-1 rounded">{{ $category->code }}</code>
                        </x-ui-table-cell>
                        <x-ui-table-cell compact="true">
                            <a href="{{ route('organization.settings.interlink-categories.show', $category) }}" class="link font-medium">
                                {{ $category->name }}
                            </a>
                        </x-ui-table-cell>
                        <x-ui-table-cell compact="true">
                            <x-ui-badge variant="{{ $category->is_active ? 'success' : 'muted' }}">
                                {{ $category->is_active ? 'Aktiv' : 'Inaktiv' }}
                            </x-ui-badge>
                        </x-ui-table-cell>
                        <x-ui-table-cell compact="true">
                            <a href="{{ route('organization.settings.interlink-categories.show', $category) }}" class="text-[var(--ui-primary)] hover:underline text-sm">
                                Bearbeiten
                            </a>
                        </x-ui-table-cell>
                    </x-ui-table-row>
    ...
    <x-ui-modal wire:model="modalShow" size="lg">
        <x-slot name="header">
            Neue Interlink-Kategorie erstellen
        </x-slot>

        <div class="space-y-4">
            <form wire:submit.prevent="store" class="space-y-4">
                <x-ui-input-text name="name" label="Name" wire:model.live="form.name" required placeholder="Name der Kategorie" />
