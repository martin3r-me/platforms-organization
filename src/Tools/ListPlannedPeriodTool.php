...
class ListPlannedPeriodTool implements ToolContract, ToolMetadataContract
{
    ...
    {
        return 'organization.planned_period.GET';
    }

    public function getDescription(): string
    {
        return 'GET /organization/planned-period - Listet Soll-Zeiträume (geplante Start-/Enddaten) mit flexiblen Filtern (Kontext, User, aktiv-Status). Unterstützt cross-team Abfragen. Zeigt modulübergreifend alle Soll-Zeiträume (Projekte, Sprints, Change-Projekte etc.).';
    }
        ...
        return $this->mergeSchemas(
            ...
            [
                'properties' => [
                    ...
                    'context_type' => [
                        'type' => 'string',
                        'description' => 'Optional: Filter nach Kontext-Typ. Kurzformen: ' . implode(', ', array_map(fn($s) => '"'.$s.'"', ContextTypeRegistry::shortNames())) . '.',
                    ],
                    'context_id' => [
                        'type' => 'integer',
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = OrganizationTimePeriod::query()
            ...
            $this->applyStandardSort($query, $arguments, ['planned_start', 'planned_end', 'created_at', 'id'], 'created_at', 'desc');
            ...
            $items = $entries->map(function (OrganizationTimePeriod $entry) use ($labelResolver) {
                $contextLabel = null;
            ...
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Soll-Zeiträume: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'read',
            'tags' => ['organization', 'planned_period', 'list', 'time_planning'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => true,
            'risk_level' => 'safe',
