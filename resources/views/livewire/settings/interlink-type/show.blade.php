<x-ui-page>
    ...
    <x-slot name="actionbar">
        <x-ui-page-actionbar :breadcrumbs="[
            ...
            ['label' => 'Settings'],
            ['label' => 'Interlink-Typen', 'href' => route('organization.settings.interlink-types.index')],
            ['label' => $interlinkType->name],
        ]">
            @if($this->isDirty)
            ...
            <x-ui-confirm-button
                ...
                size="sm"
                wire:click="delete"
                confirm-text="Interlink-Typ wirklich löschen?"
            >
                    ...
                    <div class="space-y-3">
                        <div class="py-3 px-4 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            ...
                            <div class="text-sm font-medium text-[var(--ui-secondary)]">
                                <code>{{ $interlinkType->code }}</code>
                            </div>
                        </div>
                        @if($interlinkType->description)
                            <div class="py-3 px-4 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                                <span class="text-xs text-[var(--ui-muted)]">Beschreibung</span>
                                <div class="text-sm text-[var(--ui-secondary)]">{{ $interlinkType->description }}</div>
                            </div>
                        @endif
                        <div class="py-3 px-4 bg-[var(--ui-muted-5)] rounded-lg border border-[var(--ui-border)]/40">
                            <span class="text-xs text-[var(--ui-muted)]">Erstellt</span>
                            <div class="text-sm font-medium text-[var(--ui-secondary)]">{{ $interlinkType->created_at->format('d.m.Y H:i') }}</div>
                        </div>
                    </div>
                </div>
            </div>
